...
import { HERO_ELS, type LandingEl } from "./LandingData";
...
/* ── handle field with availability check ── */
export function HandleField({ big }: { big?: boolean }) {
  ...
  function onChange(raw: string) {
    ...
    if (RESERVED.has(h)) {
      setStatus("checking");
      tRef.current = setTimeout(() => setStatus("taken"), 480);
      return;
    }
    setStatus("checking");
  ...
  return (
    <div style={{ width: "100%", maxWidth: big ? 460 : 420 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 13, alignItems: "center", flexWrap: "wrap" }}>
        <Link href="/login" className="btn btn-primary" style={status === "ok" ? { background: "rgba(212,240,196,0.12)", borderColor: "rgba(212,240,196,0.4)", color: "#fff" } : {}}>
          claim your space →
        </Link>
...
/* ── single stage element with parallax ── */
function StageEl({ el, mouse, motion }: { el: LandingEl; mouse: { x: number; y: number }; motion: boolean }) {
...
/* ── mobile stage: stacked room ── */
function HeroStageMobile() {
  const byId: Record<string, LandingEl> = Object.fromEntries(HERO_ELS.map((e) => [e.id, e]));
  return (
