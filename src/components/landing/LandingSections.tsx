...
import { GB_FEED, GALLERY, PLANS, type GallerySpace, type Plan } from "./LandingData";
  ...
  return (
    <header style={{
      ...
      <nav style={{ display: "flex", alignItems: "center", gap: "clamp(14px,2.5vw,26px)" }}>
        ...
        <Link href="/login" className="btn btn-ghost desk-only" style={{ padding: 0 }}>sign in</Link>
        <Link href="/login" className="btn btn-primary" style={{ padding: "10px 18px" }}>claim your space</Link>
      </nav>
    </header>
  );
}
...
/* ─────────────────── Gallery Section ─────────────────── */
function miniStyle(e: Record<string, any>): React.CSSProperties {
...
function MiniTileInner({ cls }: { cls: string }) {
  ...
  if (cls === "ascii") return (
    <pre style={{ margin: 0, fontFamily: "var(--mono)", fontSize: 5, lineHeight: 1.05, color: "#2a2a2a", padding: 4, whiteSpace: "pre" }}>{`  /\\_/\\\n ( o.o )\n  > ^ <\n /|   |\\\n  ^   ^`}</pre>
  );
...
function MiniSpace({ s }: { s: GallerySpace }) {
  ...
  const px = hov ? (m.x - 0.5) * -16 : 0;
  const py = hov ? (m.y - 0.5) * -10 : 0;
  return (
  ...
  return (
    <section id="gallery" className="section reveal" style={{ padding: "110px 48px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 16, marginBottom: 40 }}>
        ...
        <Link href="/browse" className="btn btn-ghost" style={{ padding: 0 }}>see all spaces →</Link>
      </div>
...
function PlanCard({ p }: { p: Plan }) {
  return (
    <div style={{
      ...
      <Link href="/login" className="btn btn-primary" style={{ marginTop: 24, width: "100%", ...(p.featured ? { background: "rgba(212,240,196,0.12)", borderColor: "rgba(212,240,196,0.4)", color: "#fff" } : {}) }}>{p.cta}</Link>
    </div>
  );
}
