...
import type { GBEntry, LandingEl } from "./LandingData";

/* ── gradient avatar ── */
export function Avatar({ art = "art-dusk", size = 64, ring = "rgba(255,255,255,0.15)" }: { art?: string; size?: number; ring?: string }) {
  return (
...
/* ── art image tile ── */
export function ImageEl({ data }: { data: any }) {
  return (
...
/* ── animated gif tile ── */
export function GifEl({ data }: { data: any }) {
  const kind: string = data.gif || "gif-aura";
  return (
...
/* ── film-framed photograph ── */
export function PhotoEl({ data }: { data: any }) {
  return (
...
export function CodeEl({ data }: { data: any }) {
  const lines: string[] = data.lines || CODE_LINES;
  return (
...
/* ── profile card ── */
export function ProfileCardEl({ data }: { data: any }) {
  const [following, setFollowing] = useState<boolean>(data.following ?? false);
  return (
    <div className="cel" style={{ width: data.w, height: data.h }}>
      ...
      <div style={{ position: "relative", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", padding: "26px 18px 18px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: "auto", paddingTop: 16, width: "100%", justifyContent: "center" }}>
          <button onClick={() => setFollowing((f: boolean) => !f)}
...
/* ── guestbook ── */
export function GuestbookEl({ data }: { data: any }) {
  const msgs: GBEntry[] = data.messages || [];
  return (
    <div className="cel" style={{ width: data.w, height: data.h }}>
...
/* ── media card (faux now-playing) ── */
export function MediaEl({ data }: { data: any }) {
  const [t, setT] = useState<number>(38);
  useEffect(() => {
    const id = setInterval(() => setT((v: number) => (v + 0.4) % 100), 240);
...
/* ── sticky note ── */
export function NoteEl({ data }: { data: any }) {
  return (
...
/* ── link pill ── */
export function LinkPillEl({ data }: { data: any }) {
  return (
...
/* ── editorial text ── */
export function TextEl({ data }: { data: any }) {
  return (
...
/* ── glitter sticker ── */
export function StickerEl({ data }: { data: any }) {
  return (
    <div className="cel" style={{ width: data.w, height: data.h }}>
...
/* ── kaomoji ── */
export function KaoEl({ data }: { data: any }) {
  return (
...
/* ── welcome banner ── */
export function BannerEl({ data }: { data: any }) {
  return (
...
/* ── dispatcher ── */
export function CanvasEl({ el }: { el: LandingEl }) {
  switch (el.type) {
