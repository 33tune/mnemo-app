  ...
  function handleDragMove(e: React.MouseEvent) {
    ...
    if (resizing) {
      ...
      if (resizing.type === "group") {
        ...
        lastResize.current = { w: Math.max(200,resizeStart.current.w+dx), h: Math.max(260,resizeStart.current.h+dy) };
        setElements(p=>p.map(el=>el.id===resizing.id&&el.elementType==="guestbook"?{...el,w:Math.max(200,resizeStart.current.w+dx),h:Math.max(260,resizeStart.current.h+dy)}:el));
      }
    }
  }
