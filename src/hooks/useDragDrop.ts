...
export type DragTarget   = { type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook"|"guestbook"; id: string };
export type ResizeTarget = { type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook"|"guestbook"; id: string } | { type: "group" };
export type RotateTarget = {
  type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook"|"guestbook";
...
export type DragUpResult = {
  ...
  resized: { id: string; type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook"|"guestbook"; w?: number; h?: number; size?: number } | null;
};
...
export function useDragDrop({
  ...
  function startDrag(
    id: string,
    type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook",
    x: number, y: number,
    e: React.MouseEvent,
  ...
  function startSingleResize(id: string, type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook", e: React.MouseEvent) {
    e.preventDefault(); e.stopPropagation();
  ...
  function handleDragMove(e: React.MouseEvent) {
    ...
    if (resizing) {
      ...
      } else if (resizing.type==="postit") {
        lastResize.current = { w: Math.max(220,resizeStart.current.w+dx), h: Math.max(200,resizeStart.current.h+dy) };
        setElements(p=>p.map(el=>el.id===resizing.id&&el.elementType==="postit"?{...el,w:Math.max(220,resizeStart.current.w+dx),h:Math.max(200,resizeStart.current.h+dy)}:el));
  ...
  function handleDragUp(selectedIds: Set<string>): DragUpResult {
    ...
    const resized = (resizing && resizing.type !== "group")
      ? { id: resizing.id, type: resizing.type as "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"|"guestbook", ...lastResize.current }
      : null;
