...
export type DragTarget   = { type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"; id: string };
export type ResizeTarget = { type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"; id: string } | { type: "group" };
export type RotateTarget = {
  type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media";
...
export type DragUpResult = {
  ...
  resized: { id: string; type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media"; w?: number; h?: number; size?: number } | null;
};
...
export function useDragDrop({
  ...
  function startSingleResize(id: string, type: "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media", e: React.MouseEvent) {
    e.preventDefault(); e.stopPropagation();
  ...
  function handleDragUp(selectedIds: Set<string>): DragUpResult {
    ...
    const resized = (resizing && resizing.type !== "group")
      ? { id: resizing.id, type: resizing.type as "image"|"card"|"text"|"gallery"|"profile"|"postit"|"media", ...lastResize.current }
      : null;
