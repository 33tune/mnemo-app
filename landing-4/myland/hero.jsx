/* ════════ myLand — HERO: living canvas + parallax + list→space bloom ════════ */
const { useState: useStateH, useEffect: useEffectH, useRef: useRefH } = React;

const RESERVED = new Set(["neve", "admin", "me", "myland", "home", "tomo", "sol"]);
const STAGE_W = 1240, STAGE_H = 720, CX = STAGE_W / 2, CY = STAGE_H / 2;

function useViewport() {
  const [v, setV] = useStateH({ w: typeof window !== "undefined" ? window.innerWidth : 1280, h: typeof window !== "undefined" ? window.innerHeight : 800 });
  useEffectH(() => {
...
/* ── handle field with simulated availability check ── */
function HandleField({ big }) {
  const [val, setVal] = useStateH("");
  const [status, setStatus] = useStateH("idle"); // idle|checking|ok|taken|invalid
  const tRef = useRefH(null);
...
/* ── one canvas element: always visible, with parallax (outer) + ambient float (inner) ── */
function StageEl({ el, mouse, motion }) {
  const px = motion ? mouse.x * el.depth * 30 : 0;
  const py = motion ? mouse.y * el.depth * 30 : 0;
  const floatCls = motion && (el.type === "gif" || el.type === "note" || el.type === "sticker" || el.type === "kao") ? "floaty" : "";
  return (
    <div style={{ position: "absolute", left: el.x, top: el.y, transform: `translate(${px}px,${py}px)`, willChange: "transform", zIndex: el.z != null ? el.z : Math.round(el.depth * 10) }}>
      <div className={floatCls} style={{ transform: `rotate(${el.rot || 0}deg)`, "--r": (el.rot || 0) + "deg" }}>
        <CanvasEl el={el} />
      </div>
    </div>
  );
...
/* ── desktop stage: neve's room, always present, gentle parallax ── */
function HeroStage({ vp }) {
  const [mouse, setMouse] = useStateH({ x: 0, y: 0 });
  const raf = useRefH(0);
...
/* ── mobile stage: a dense, authored room stacked vertically ── */
function HeroStageMobile() {
  const byId = Object.fromEntries(HERO_ELS.map(e => [e.id, e]));
  return (
    <div style={{ position: "relative", padding: "4px 0 30px", display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
      ...
      {/* dominant profile + pinned star + kao */}
      ...
      {/* guestbook + scraps pinned over edges */}
...
function Hero() {
  ...
  return (
    <section id="hero" style={{ position: "relative", minHeight: mobile ? "auto" : "100vh", overflow: "hidden", paddingTop: mobile ? 78 : 0 }}>
      ...
      <style>{`.floaty{animation:driftA 6s ease-in-out infinite}`}</style>
    </section>
  );
}

Object.assign(window, { Hero, HandleField, useViewport });
