  ...
  return (
    <header style={{
      ...
      <a href="#hero" style={{ display: "flex", alignItems: "center", gap: 11 }}>
        <img src="myland/assets/logo.png" alt="myLand" style={{ width: 34, height: "auto", filter: pulse ? "drop-shadow(0 0 10px rgba(255,64,141,0.6))" : "none", transition: "filter 1.6s ease" }} />
...
/* ───────────── The Kit (objects, not a feature grid) ───────────── */
function KitObject({ label, rot, children }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12 }}>
      <div className="floaty2" style={{ transform: `rotate(${rot}deg)`, "--r": rot + "deg" }}>{children}</div>
...
function KitSection() {
  return (
    <section className="section reveal" style={{ padding: "120px 48px 110px", textAlign: "center" }}>
      ...
      <style>{`.floaty2{animation:floatY 7s ease-in-out infinite}.floaty2:nth-child(2n){animation-delay:-2s}.floaty2:nth-child(3n){animation-delay:-4s}`}</style>
    </section>
  );
}
        ...
        <div className="gb-grid" style={{ display: "grid", gap: 56, alignItems: "center" }}>
          ...
          <div style={{ position: "relative", display: "flex", justifyContent: "center", minHeight: 420 }}>
            <div style={{ position: "relative", width: 300, height: 420 }}>
              ...
              <div className="floaty2" style={{ position: "absolute", top: -26, right: -40, transform: "rotate(8deg)", "--r": "8deg", zIndex: 5 }}>
                <NoteEl data={{ tone: "pink", text: "happy bday!! 🎂", size: 13, w: 130, h: 80 }} />
              </div>
              <div className="floaty2" style={{ position: "absolute", bottom: 30, left: -52, transform: "rotate(-7deg)", "--r": "-7deg", zIndex: 5 }}>
                <GifEl data={{ gif: "gif-aura", w: 92, h: 92 }} />
              </div>
              <div className="floaty2" style={{ position: "absolute", bottom: -24, right: -30, transform: "rotate(5deg)", "--r": "5deg", zIndex: 5 }}>
...
/* ───────────── Gallery — live mini rooms ───────────── */
function miniStyle(e) {
  const base = { position: "absolute", left: e.x, top: e.y, width: e.w, height: e.h, borderRadius: e.r ?? 6, transform: `rotate(${e.t || 0}deg)`, overflow: "hidden" };
  ...
  return { ...base, boxShadow: "0 8px 18px -10px rgba(0,0,0,0.55)" }; // art-*/gif-*/scene-*/riso-* via className
}
...
function MiniTileInner({ cls }) {
  if (cls === "gif-aura") return <div className="spin" />;
  if (cls === "gif-scan") return <div className="line" />;
  if (cls === "code") return <MiniCode />;
  if (cls === "grid6") return <React.Fragment>{GRID6.map((g, i) => <span key={i} className={"art " + g} style={{ borderRadius: 2 }} />)}</React.Fragment>;
  if (cls === "mediawin") return (
    <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ height: 12, background: "#000", display: "flex", alignItems: "center", paddingLeft: 5, gap: 3 }}>
        <span style={{ width: 4, height: 4, borderRadius: "50%", background: "#ff5a5a" }} /><span style={{ fontFamily: "var(--mono)", fontSize: 5, color: "rgba(255,255,255,0.6)" }}>MEDIA</span>
      </div>
...
function MiniSpace({ s }) {
  const [hov, setHov] = useS(false);
  const [m, setM] = useS({ x: 0.5, y: 0.5 });
  const onMove = e => { const r = e.currentTarget.getBoundingClientRect(); setM({ x: (e.clientX - r.left) / r.width, y: (e.clientY - r.top) / r.height }); };
  ...
  return (
    <section id="claim" className="reveal gridlines" style={{ position: "relative", padding: "150px 48px 70px", textAlign: "center", overflow: "hidden" }}>
      ...
      <div style={{ position: "relative", maxWidth: 760, margin: "0 auto" }}>
        <img src="myland/assets/logo.png" alt="myLand" style={{ width: 84, height: "auto", margin: "0 auto 22px", display: "block", filter: "drop-shadow(0 8px 26px rgba(255,64,141,0.32))" }} />
      ...
      <footer style={{ position: "relative", marginTop: 130, paddingTop: 26, borderTop: "1px solid var(--border)", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16, maxWidth: 1144, marginInline: "auto" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 9 }}>
          <img src="myland/assets/logo.png" alt="" style={{ width: 28, height: "auto" }} />
...
Object.assign(window, { Topbar, KitSection, GuestbookSection, GallerySection, PricingSection, Closing });
